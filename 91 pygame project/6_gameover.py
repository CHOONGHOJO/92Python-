...
balls.append({
  ...
  "to_x":3, # 공의 x 축 이동 방향 -3 이면 왼쪽으로 3 이면 오른쪽으로
  "to_y":-6, # y축 이동 방향
...
# 게임 종료 메시지
# Mission Complete(성공), Tiem Over(시간 초과, 실패), Game Over(캐릭터 공에 맞음, 실패)
...
while running:
  ...
  for ball_idx, ball_val in enumerate(balls):
    ...
    else: # 그 외의 모든 경우에는 속도를 증가 (포물선 효과)
      ball_val["to_y"] += 0.5
  ...
  for ball_idx, ball_val in enumerate(balls):
    ...
    for weapon_idx, weapon_val in enumerate(weapons):
      ...
      continue # 안쩍 for문 조건이 맞지 않으면 continue, 바깥 for문 게속 수행
    break # 안쪽 for 문에서 break 만나면 여기로 진입 가능, 2중 for문을 한번에 탈출

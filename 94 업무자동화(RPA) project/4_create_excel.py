...
with smtplib.SMTP("smtp.gmail.com", 587) as smtp:
  ...
  for applicant in applicant_list:
    ...
    # phone = applicant[3]
    index, nickname, phone = applicant[1:] # 1번째 값을 무시한 후
    ...
    if index <= max_val:
      title = "파이썬 특강 안내 [선정]"
      content = "{}님 축하드립니다, 특강 대상자로 선정 되셨습니다. (선정순번 {}번)".format(nickname, index)
    else:
      title = "파이썬 특강 안내 [탈락]"
...
print("[3. 선정자 명단 EXCEL FILE 생성")
wb = Workbook()
ws = wb.active()
ws.append(["순번", "닉네임", "전화번호"])

for applicant in applicant_list[:max_val]: # 0:3 => 0~2
  # index = applicant[1]
  # nickname = applicant[2]
  # phone = applicant[3]
